...
let scripts = 0;
let pages = 0;
for (const file of [...fs.readdirSync(path.join(root, 'js')).map(name => `js/${name}`), 'sw.js']) {
...
for (const file of fs.readdirSync(root).filter(name => name.endsWith('.html'))) {
  ...
  for (const [, code] of html.matchAll(/<script\b[^>]*>([\s\S]*?)<\/script>/g)) {
    if (code.trim()) new vm.Script(code, { filename: file });
  }
  pages++;
}
console.log(`Checked ${scripts} scripts and ${pages} pages: syntax, duplicate IDs and local references pass.`);
